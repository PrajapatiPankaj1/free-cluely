...
const Queue: React.FC<QueueProps> = ({ setView }) => {
  ...
  return (
    <div
      ref={(el) => {
        if (barRef) (barRef as any).current = el
            ...
            <div className="flex-1 overflow-y-auto mb-3 p-3 rounded-lg bg-white/10 backdrop-blur-md max-h-64 min-h-[120px] glass-content border border-white/20 shadow-lg">
              {chatMessages.length === 0 ? (
                ...
                chatMessages.map((msg, idx) => (
                  ...
                  >
                    <div
                      className={`max-w-[85%] px-3 py-2 rounded-xl text-xs shadow-md backdrop-blur-sm border ${
                        msg.role === "user" 
                          ? "bg-gray-700/80 text-gray-100 ml-12 border-gray-600/40" 
                          : "bg-white/90 text-gray-800 mr-12 border-gray-200/50 font-medium"
                      }`}
                      style={{ whiteSpace: "pre-wrap", wordBreak: "break-word", lineHeight: "1.5" }}
                    >
                      {msg.text.replace(/\*\*/g, '').replace(/###\s*\d*\.?\s*/g, '').replace(/---\s*/g, '')}
                    </div>
                  </div>
                ))
              )}
